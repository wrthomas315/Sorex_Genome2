...
# Where RepeatMasker for centos8 is stored
REPEATMASKER = "/lustre/work/daray/software/RepeatMasker-4.1.0-centos8"
...
REPEATMASKERPATH = "/lustre/work/daray/software/RepeatMasker-4.1.0-centos8"
SOFTWARE = '/lustre/work/daray/software'

#Function 1: build doLift.sh
def buildDoLift(GENOME_NAME, OUTDIR):
	...
	OUT.write('  [ -f $d0/$bNum.fa.align ] && $SOFTWARE/ucscTools/liftUp -type=.align stdout $d0/$bNum.lft error $d0/$bNum.fa.align > $d0/$bNum.fa.liftedAlign\n')
	OUT.write('done\n\n')
	OUT.write('$SOFTWARE/ucscTools/liftUp ' + GENOME_NAME + '.fa.out /dev/null carry RMPart/???/*.liftedOut\n')
	OUT.write('$SOFTWARE/ucscTools/liftUp ' + GENOME_NAME + '.fa.align /dev/null carry RMPart/???/*.liftedAlign\n\n')
	...
	OUT.write('perl $REPEATMASKERPATH/util/calcDivergenceFromAlign.pl -s ' + GENOME_NAME + '.divsum ' + GENOME_NAME + '.fa.align\n')
	OUT.write('perl $REPEATMASKERPATH/util/createRepeatLandscape.pl -div ' + GENOME_NAME + '.divsum -twoBit ' + GENOME_NAME + '.2bit > ' + GENOME_NAME + '-landscape.html\n')
	...
	OUT.write('#then $REPEATMASKERPATH/util/rmToUCSCTables.pl -out ' + GENOME_NAME + '.fa.out -align ' + GENOME_NAME + '.fa.align\n')
	OUT.write('#fi\n')
...
#Define KUE for use in generating scripts. Not needed in SLURM version.
#if QUEUE == 'hrothgar':
